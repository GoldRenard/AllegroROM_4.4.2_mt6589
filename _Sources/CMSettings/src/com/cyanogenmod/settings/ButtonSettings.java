...
public class ButtonSettings extends SettingsPreferenceFragment implements
    ...
    private static final String PERSIST_BUTTON_BRIGHTNESS = "button_brightness_mode";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        final int deviceKeys = getResources().getInteger(R.integer.config_deviceHardwareKeys);
        ...
        if (hasHomeKey) {
            if (!res.getBoolean(R.bool.config_show_homeWake)) {
                homeCategory.removePreference(findPreference(SettingsWrapper.System.HOME_WAKE_SCREEN));
            }
            ...
            int longPressAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_HOME_LONG_PRESS_ACTION,
            ...
            int doubleTapAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_HOME_DOUBLE_TAP_ACTION,
        ...
        if (hasMenuKey) {
            int pressAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_MENU_ACTION, ACTION_MENU);
            mMenuPressAction = initActionList(KEY_MENU_PRESS, pressAction);

            int longPressAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_MENU_LONG_PRESS_ACTION,
        ...
        if (hasAssistKey) {
            int pressAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_ASSIST_ACTION, ACTION_SEARCH);
            mAssistPressAction = initActionList(KEY_ASSIST_PRESS, pressAction);

            int longPressAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_ASSIST_LONG_PRESS_ACTION, ACTION_VOICE_SEARCH);
        ...
        if (hasAppSwitchKey) {
            int pressAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_APP_SWITCH_ACTION, ACTION_APP_SWITCH);
            mAppSwitchPressAction = initActionList(KEY_APP_SWITCH_PRESS, pressAction);

            int longPressAction = Settings.System.getInt(resolver,
            		SettingsWrapper.System.KEY_APP_SWITCH_LONG_PRESS_ACTION, ACTION_NOTHING);
        ...
        if (hasCameraKey) {
            mCameraWake = (CheckBoxPreference)
                prefScreen.findPreference(SettingsWrapper.System.CAMERA_WAKE_SCREEN);
            mCameraSleepOnRelease = (CheckBoxPreference)
                prefScreen.findPreference(SettingsWrapper.System.CAMERA_SLEEP_ON_RELEASE);
            mCameraMusicControls = (CheckBoxPreference)
                prefScreen.findPreference(SettingsWrapper.System.CAMERA_MUSIC_CONTROLS);
            boolean value = mCameraWake.isChecked();
            mCameraMusicControls.setEnabled(!value);
            mCameraSleepOnRelease.setEnabled(value);
            if (getResources().getBoolean(R.bool.config_singleStageCameraKey)) {
        ...
        if (Utils.hasVolumeRocker(getActivity())) {
            ...
            if (!Utils.hideUnsupportedFeatures()) {
            	int swapVolumeKeys = Settings.System.getInt(getContentResolver(),
                		SettingsWrapper.System.SWAP_VOLUME_KEYS_ON_ROTATION, 0);
            ...
            if (!res.getBoolean(R.bool.config_show_volumeRockerWake)) {
                volumeCategory.removePreference(findPreference(SettingsWrapper.System.VOLUME_WAKE_SCREEN));
            }
            
            mVolumeOverlay = (ListPreference) findPreference(KEY_VOLUME_OVERLAY);
            mVolumeOverlay.setOnPreferenceChangeListener(this);
            int volumeOverlay = Settings.System.getInt(getContentResolver(),
                    SettingsWrapper.System.MODE_VOLUME_OVERLAY, 1);
    	...
    	updateButtonBrightnessState();

        Utils.updatePreferenceToSpecificActivityFromMetaDataOrRemove(getActivity(),
                getPreferenceScreen(), KEY_BLUETOOTH_INPUT_SETTINGS);
    }
    
    private void updateButtonBrightnessState() {
    	int mode = Settings.System.getInt(getContentResolver(), PERSIST_BUTTON_BRIGHTNESS, 0);
    	if (mButtonBrightness != null) {
    		mButtonBrightness.setValue(String.valueOf(mode));
    		mButtonBrightness.setSummary(mButtonBrightness.getEntry());
    	}
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mHomeLongPressAction) {
            ...
            handleActionListChange(mMenuPressAction, newValue,
            		SettingsWrapper.System.KEY_MENU_ACTION);
            return true;
            ...
            handleActionListChange(mAssistPressAction, newValue,
            		SettingsWrapper.System.KEY_ASSIST_ACTION);
            return true;
        } else if (preference == mAssistLongPressAction) {
            handleActionListChange(mAssistLongPressAction, newValue,
            		SettingsWrapper.System.KEY_ASSIST_LONG_PRESS_ACTION);
            return true;
        } else if (preference == mAppSwitchPressAction) {
            handleActionListChange(mAppSwitchPressAction, newValue,
            		SettingsWrapper.System.KEY_APP_SWITCH_ACTION);
            return true;
        } else if (preference == mAppSwitchLongPressAction) {
            handleActionListChange(mAppSwitchLongPressAction, newValue,
            		SettingsWrapper.System.KEY_APP_SWITCH_LONG_PRESS_ACTION);
            return true;
        } else if (preference == mVolumeKeyCursorControl) {
            handleActionListChange(mVolumeKeyCursorControl, newValue,
            		SettingsWrapper.System.VOLUME_KEY_CURSOR_CONTROL);
            return true;
        } else if (preference == mButtonBrightness) {
        	handleActionListChange(mButtonBrightness, newValue,
            		PERSIST_BUTTON_BRIGHTNESS);
            return true;
            ...
            Settings.System.putInt(getContentResolver(),
                    SettingsWrapper.System.MODE_VOLUME_OVERLAY, value);
    ...
    private static void writeDisableNavkeysOption(Context context, boolean enabled) {
        ...
        final int defaultBrightness = context.getResources().getInteger(
        		R.integer.config_buttonBrightnessSettingDefault);

        Settings.System.putInt(context.getContentResolver(),
                SettingsWrapper.System.DEV_FORCE_SHOW_NAVBAR, enabled ? 1 : 0);
        //KeyDisabler.setActive(enabled);

        try {
	        Runtime.getRuntime().exec(new String[]{"su","-c","mount -o rw,remount /system"});
	    	Runtime.getRuntime().exec(new String[]{"su","-c","sed -i s/qemu.hw.mainkeys=.*/qemu.hw.mainkeys=" + (enabled ? "0" : "1") + "/ /system/build.prop"});
        ...
        if (enabled) {
            int currentBrightness = Settings.System.getInt(context.getContentResolver(),
            		SettingsWrapper.System.BUTTON_BRIGHTNESS, defaultBrightness);
            if (!prefs.contains("pre_navbar_button_backlight")) {
                editor.putInt("pre_navbar_button_backlight", currentBrightness);
            }
            Settings.System.putInt(context.getContentResolver(),
            		SettingsWrapper.System.BUTTON_BRIGHTNESS, 0);
        } else {
            Settings.System.putInt(context.getContentResolver(),
            		SettingsWrapper.System.BUTTON_BRIGHTNESS,
    ...
    private void updateDisableNavkeysOption() {
        boolean enabled = Settings.System.getInt(getActivity().getContentResolver(),
        		SettingsWrapper.System.DEV_FORCE_SHOW_NAVBAR, 0) != 0;
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mSwapVolumeButtons) {
            ...
            Settings.System.putInt(getActivity().getContentResolver(),
                    SettingsWrapper.System.SWAP_VOLUME_KEYS_ON_ROTATION, value);
