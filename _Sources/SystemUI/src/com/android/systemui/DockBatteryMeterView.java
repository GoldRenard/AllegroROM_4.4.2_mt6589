...
import android.os.BatteryManagerWrapper;
...
public class DockBatteryMeterView extends BatteryMeterView {
    ...
    private class DockBatteryTracker extends BatteryTracker {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                if (mSupported) {
                    present = intent.getBooleanExtra(BatteryManagerWrapper.EXTRA_DOCK_PRESENT, false);

                    level = (int)(100f
                            * intent.getIntExtra(BatteryManagerWrapper.EXTRA_DOCK_LEVEL, 0)
                            / intent.getIntExtra(BatteryManagerWrapper.EXTRA_DOCK_SCALE, 100));

                    plugType = intent.getIntExtra(BatteryManagerWrapper.EXTRA_DOCK_PLUGGED, 0);
                    plugged = plugType == BatteryManagerWrapper.BATTERY_DOCK_PLUGGED_AC;
                    health = intent.getIntExtra(BatteryManagerWrapper.EXTRA_DOCK_HEALTH,
                            BatteryManager.BATTERY_HEALTH_UNKNOWN);
                    status = intent.getIntExtra(BatteryManagerWrapper.EXTRA_DOCK_STATUS,
                            BatteryManager.BATTERY_STATUS_UNKNOWN);
                    technology = intent.getStringExtra(BatteryManagerWrapper.EXTRA_DOCK_TECHNOLOGY);
                    voltage = intent.getIntExtra(BatteryManagerWrapper.EXTRA_DOCK_VOLTAGE, 0);
                    temperature = intent.getIntExtra(BatteryManagerWrapper.EXTRA_DOCK_TEMPERATURE, 0);
                    ...
                    public void run() {
                        if (curLevel < 0) {
                            ...
                            dummy.putExtra("plugged", incr > 0
                                    ? BatteryManagerWrapper.BATTERY_DOCK_PLUGGED_AC : 0);
    ...
    public DockBatteryMeterView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mBatteryService = ((BatteryManager) context.getSystemService("battery"));
