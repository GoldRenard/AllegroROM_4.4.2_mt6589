...
public class SettingsProvider extends ContentProvider
{
  ...
  private static final SettingsCache sGlobalCache;
  private static final SparseArray<AtomicInteger> sKnownMutationsInFlight;
  private static SparseArray<SettingsFileObserver> sObserverInstances = new SparseArray();
  ...
  static
  {
    sSecureCaches = new SparseArray();
    sGlobalCache = new SettingsCache("global");
    sKnownMutationsInFlight = new SparseArray();
  ...
  private Uri getDefaultUri(Context paramContext, int paramInt)
  {
    String str;
    if (paramInt == 2) {
      str = Settings.System.getString(paramContext.getContentResolver(), "mtk_audioprofile_default_notification");
    }
    if (paramInt == 32)
    {
      str = Settings.System.getString(paramContext.getContentResolver(), "mtk_audioprofile_default_sms_noti");
    }
    else if (paramInt == 128)
    {
      str = Settings.System.getString(paramContext.getContentResolver(), "mtk_audioprofile_default_sms_noti_2");
    }
    else if (paramInt == 64)
    {
      str = Settings.System.getString(paramContext.getContentResolver(), "mtk_audioprofile_default_ringtone_2");
    }
    else if (paramInt == 1)
    {
      str = Settings.System.getString(paramContext.getContentResolver(), "mtk_audioprofile_default_ringtone");
    }
    else if (paramInt == 8)
    {
      str = Settings.System.getString(paramContext.getContentResolver(), "mtk_audioprofile_default_video_call");
    }
    else if (paramInt == 4)
    {
      str = Settings.System.getString(paramContext.getContentResolver(), "mtk_audioprofile_default_alarm");
    }
    else
    {
      Xlog.d("SettingsProvider", "getDefaultUri: unexpectedType = " + paramInt);
      return null;
    }
  ...
  private boolean initDialogSequenceValue(int paramInt)
  {
    Cursor localCursor = queryForUser(Settings.System.CONTENT_URI, new String[] { "value" }, "name=?", new String[] { "dialog_sequence_settings" }, null, paramInt);
    try
    {
      if (localCursor.moveToNext());
      for (String str = localCursor.getString(0); str != null; str = null)
      {
        Xlog.d("SettingsProvider", "reset DIALOG_SEQUENCE_SETTINGS to default");
        ContentValues localContentValues = new ContentValues();
        localContentValues.put("name", "dialog_sequence_settings");
        localContentValues.put("value", Integer.valueOf(0));
        Uri localUri = insertForUser(Settings.System.CONTENT_URI, localContentValues, paramInt);
        if (localUri != null)
          break;
        return false;
      }
  ...
  private boolean initOobeValue(int paramInt)
  {
    Cursor localCursor = queryForUser(Settings.System.CONTENT_URI, new String[] { "value" }, "name=?", new String[] { "oobe_display" }, null, paramInt);
    try
    {
      if (localCursor.moveToNext());
      for (String str = localCursor.getString(0); str != null; str = null)
      {
        ...
        ContentValues localContentValues = new ContentValues();
        localContentValues.put("name", "oobe_display");
        localContentValues.put("value", Integer.valueOf(0));
        Uri localUri = insertForUser(Settings.System.CONTENT_URI, localContentValues, paramInt);
        if (localUri != null)
          break;
              ...
              if (authority.equals(MediaStore.AUTHORITY)) {
                  ...
                  try {
                      ...
                      return new AssetFileDescriptor(pfd, 0, -1);
						...
						soundUri = getDefaultUri(context, ringtoneType);
                  }
              }
