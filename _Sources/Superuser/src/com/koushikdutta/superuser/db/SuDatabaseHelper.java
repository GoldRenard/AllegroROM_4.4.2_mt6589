...
public class SuDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        addDefaultPolicy(mContext, db, "com.cyanogenmod.settings");
		addDefaultPolicy(mContext, db, "com.cyanogenmod.filemanager");
		addDefaultPolicy(mContext, db, "com.allegrorom.ota");
    }
    ...
    private static void addDefaultPolicy(Context context, SQLiteDatabase db, String packageName) {
        try {
            ApplicationInfo mInfo = context.getPackageManager().getApplicationInfo(packageName, 0);
            UidPolicy policy = new UidPolicy();
            policy.logging = false;
            policy.policy = UidPolicy.ALLOW;
            policy.until = 0;
            policy.command = null;
            policy.uid = mInfo.uid;
            policy.desiredUid = 0;
            policy.packageName = packageName;
            policy.name = context.getPackageManager().getApplicationLabel(mInfo).toString();
