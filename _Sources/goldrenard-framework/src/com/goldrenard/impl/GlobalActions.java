...
public class GlobalActions implements DialogInterface.OnDismissListener, DialogInterface.OnClickListener  {
    ...
    public void showRebootDialog() {
      ...
      awakenIfNecessary();
      mRebootCount = 3;
      AlertDialog.Builder localBuilder = new AlertDialog.Builder(mContext).setTitle(R.string.power_off);
      
      String str = mContext.getString(51052569);
      String cnt = Integer.toString(mRebootCount);
      
      mRebootDialog = localBuilder.setMessage(TextUtils.expandTemplate(str, cnt)).create();
      mRebootDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
      
      WindowManager.LayoutParams lParams = mRebootDialog.getWindow().getAttributes();
      lParams.setTitle("GlobalActions");
      
      mRebootDialog.getWindow().setAttributes(lParams);
      mRebootDialog.show();
      mRebootDialog.getWindow().getDecorView().setSystemUiVisibility(65536);
      mHandler.sendEmptyMessage(MESSAGE_REBOOT);
    }
    ...
    private void handleRebootShow() {
      if (mRebootCount == 0) {
    	  cancelRebootDialog();
      }
      while (mRebootDialog != null) {
	      String str = this.mContext.getString(51052569);
	      String cnt = Integer.toString(mRebootCount);
	      mRebootDialog.setMessage(TextUtils.expandTemplate(str, cnt));
	      mRebootCount -= 1;
	      mHandler.sendEmptyMessageDelayed(MESSAGE_REBOOT, 1000L);
      }
    }
    
    public void cancelRebootDialog() {
    	mHandler.removeMessages(MESSAGE_REBOOT);
    	if (mRebootDialog != null) {
    		mRebootDialog.dismiss();
    		mRebootDialog = null;
    	}
    ...
    private GlobalActionsDialog createDialog() {
        ...
        mExpandDesktopModeOn = new ToggleAction(
        		utils.R.drawable.ic_lock_expanded_desktop,
                utils.R.drawable.ic_lock_expanded_desktop_off,
                utils.R.string.global_actions_toggle_expanded_desktop_mode,
                utils.R.string.global_actions_expanded_desktop_mode_on_status,
                utils.R.string.global_actions_expanded_desktop_mode_off_status) {
        ...
        final boolean quickbootEnabled = Settings.Global.getInt(
                mContext.getContentResolver(), SettingsWrapper.Global.ENABLE_QUICKBOOT,
        ...
        boolean showReboot = Settings.System.getIntForUser(cr,
        		SettingsWrapper.System.POWER_MENU_REBOOT_ENABLED, 1, UserHandle.USER_CURRENT) == 1;
        if (showReboot) {
            /*mItems.add(
                new SinglePressAction(utils.R.drawable.ic_lock_reboot, utils.R.string.global_action_reboot) {
                    public void onPress() {
        	...
        	mItems.add(new SinglePressAction(utils.R.drawable.ic_lock_reboot, utils.R.string.global_action_reboot) {

                private int mSel;
                public void onPress() {
                    mSel = -1;
                    AlertDialog mRebootDialog = new AlertDialog.Builder(mContext)
                    	.setTitle(utils.R.string.global_action_reboot)
                    	.setIcon(utils.R.drawable.ic_lock_reboot)
                    	.setSingleChoiceItems(utils.R.arrays.reboot_options, 0, null)
                    	...
                    	.create();
                    mRebootDialog.setOnDismissListener(new android.content.DialogInterface.OnDismissListener() {
                    	@Override
                        public void onDismiss(DialogInterface dialoginterface) {
                            ...
                            String mode = mContext.getResources().getStringArray(utils.R.arrays.reboot_modes)[mSel];
                            if (mode.equals("hotboot")) {
                            	try { Runtime.getRuntime().exec(new String[] { "pkill", "-TERM", "-f", "system_server" }); }
                            	catch (Exception ex) { }
                            } else {
        ...
        /*boolean showProfiles =
                Settings.System.getIntForUser(cr,
                		SettingsWrapper.System.SYSTEM_PROFILES_ENABLED, 1, UserHandle.USER_CURRENT) == 1
                && Settings.System.getIntForUser(cr,
                		SettingsWrapper.System.POWER_MENU_PROFILES_ENABLED, 1, UserHandle.USER_CURRENT) == 1;
        if (showProfiles) {
            mItems.add(
        ...
        boolean showScreenshot = Settings.System.getIntForUser(cr,
        		SettingsWrapper.System.POWER_MENU_SCREENSHOT_ENABLED, 0, UserHandle.USER_CURRENT) == 1;
        if (showScreenshot) {
            mItems.add(
                new SinglePressAction(utils.R.drawable.ic_lock_screenshot, utils.R.string.global_action_screenshot) {
                    public void onPress() {
                        takeScreenshot();
                    }
        ...
        if (!utils.R.hideUnsupportedFeatures()) {
	        ...
	        boolean showExpandedDesktop =
	                Settings.System.getIntForUser(cr,
	                		SettingsWrapper.System.EXPANDED_DESKTOP_STYLE, 0, UserHandle.USER_CURRENT) != 0
	                && Settings.System.getIntForUser(cr,
	                		SettingsWrapper.System.POWER_MENU_EXPANDED_DESKTOP_ENABLED, 0, UserHandle.USER_CURRENT) == 1;
        ...
        boolean showAirplaneMode = Settings.System.getIntForUser(cr,
                SettingsWrapper.System.POWER_MENU_AIRPLANE_ENABLED, 1, UserHandle.USER_CURRENT) == 1;
        if (showAirplaneMode) {
        ...
        boolean showUsers = Settings.System.getIntForUser(cr,
        		SettingsWrapper.System.POWER_MENU_USER_ENABLED, 0, UserHandle.USER_CURRENT) == 1;
        if (showUsers) {
        ...
        boolean showSoundMode = SHOW_SILENT_TOGGLE && Settings.System.getIntForUser(cr,
        		SettingsWrapper.System.POWER_MENU_SOUND_ENABLED, 1, UserHandle.USER_CURRENT) == 1;
        if (showSoundMode) {
    ...
    private void prepareDialog() {
        ...
        mDialog.setTitle(utils.R.string.global_actions);
    ...
    private static final int MESSAGE_REBOOT = 4;
    ...
    private void onExpandDesktopModeChanged() {
        boolean expandDesktopModeOn = Settings.System.getIntForUser(
                ...
                SettingsWrapper.System.EXPANDED_DESKTOP_STATE,
